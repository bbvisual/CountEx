...
class GroundingDinoObjectDetectionOutput(ModelOutput):
    ...
    enc_outputs_coord_logits: Optional[torch.FloatTensor] = None
    spatial_shapes: Optional[torch.FloatTensor] = None
    positive_feature_maps: Optional[Tuple[torch.FloatTensor]] = None
    negative_feature_maps: Optional[Tuple[torch.FloatTensor]] = None
    density_map_pred: Optional[torch.FloatTensor] = None
    extra_loss: Optional[torch.FloatTensor] = None
...
class GroundingDinoConvEncoder(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor, pixel_mask: torch.Tensor):
        ...
        for feature_map in features:
            ...
            mask = nn.functional.interpolate(pixel_mask[None].to(torch.bfloat16), size=feature_map.shape[-2:]).to(torch.bool)[0]
...
class GroundingDinoConvModel(nn.Module):
    ...
    def forward(self, pixel_values, pixel_mask):
        ...
        # print('conv_encoder: ', self.conv_encoder)
...
def multi_scale_deformable_attention(
    ...
    for level_id, (height, width) in enumerate(value_spatial_shapes):
        ...
        sampling_grid_l_ = sampling_grid_l_.to(torch.bfloat16)
...
class GroundingDinoEncoderLayer(nn.Module):
    ...
    def get_text_position_embeddings(
        ...
        if text_position_embedding is None and text_position_ids is None:
            ...
            text_position_embedding = text_position_embedding.to(torch.bfloat16)
...
class GroundingDinoDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # print("text_encoder_hidden_states: ", text_encoder_hidden_states.shape)
        # print("text_encoder_attention_mask: ", text_encoder_attention_mask.shape)
...
class GroundingDinoContrastiveEmbedding(nn.Module):
    ...
    def forward(self, 
        ...
        # print("res: ", res.shape)
        # print("vision_hidden_state: ", vision_hidden_state.shape)
        # print("y: ", y.shape)
...
class GroundingDinoDecoder(GroundingDinoPreTrainedModel):
    ...
    def forward(
        ...
        # print("inital text_encoder_attention_mask: ", text_encoder_attention_mask.shape, text_encoder_attention_mask)

        if text_encoder_attention_mask is not None:
            # exemplar_text_encoder_attention_mask = torch.zeros((1, 3), dtype=torch.bool, device=text_encoder_attention_mask.device)
            # text_encoder_attention_mask = torch.cat([text_encoder_attention_mask, exemplar_text_encoder_attention_mask], dim=1)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            query_pos = query_pos.to(torch.bfloat16)
...
class GroundingDinoModel(GroundingDinoPreTrainedModel):
    ...
    def get_valid_ratio(self, mask):
        ...
        #valid_ratio_heigth = valid_ratio_heigth.to(torch.bfloat16)
        #valid_ratio_width = valid_ratio_width.to(torch.bfloat16)
    ...
    def forward(
        ...
        **kwargs,
        ...
        exemplars = kwargs.get('exemplars', None)
        ...
        if self.config.num_feature_levels > len(feature_maps):
            ...
            for level in range(_len_sources, self.config.num_feature_levels):
                ...
                mask = nn.functional.interpolate(pixel_mask[None].to(torch.bfloat16), size=source.shape[-2:]).to(torch.bool)[0]
        ...
        for level, (source, mask, pos_embed) in enumerate(zip(feature_maps, masks, position_embeddings_list)):
            ...
            # get the tokens corresponding to the exemplars
            cur_level_exemplar_tokens = []
            if exemplars is not None:
                b, d, h, w = source.shape
                for exemplar_idx in range(exemplars.shape[0]):
                    tlx, tly, brx, bry = exemplars[exemplar_idx]
                    tlx = int(tlx * w)
                    tly = int(tly * h)
                    brx = int(brx * w)
                    bry = int(bry * h)
                    assert tlx >= 0 and tly >= 0 and brx <= w and bry <= h and tlx <= brx and tly <= bry
                    # get the tokens corresponding to the exemplar
                    bry = min(bry + 1, h)
                    brx = min(brx + 1, w)
                    # b, d, h, w
                    exemplar_tokens = source[:, :, tly:bry, tlx:brx]
                    # 1, 4, 256
                    exemplar_tokens = exemplar_tokens.permute(0, 2, 3, 1)
                    exemplar_tokens = exemplar_tokens.flatten(start_dim=1, end_dim=2)
                    # mean pooling over dim 1
                    exemplar_tokens = exemplar_tokens.mean(dim=1, keepdim=True)
                    # print("exemplar_tokens: ", exemplar_tokens.shape)
                    cur_level_exemplar_tokens.append(exemplar_tokens)
                cur_level_exemplar_tokens = torch.cat(cur_level_exemplar_tokens, dim=1)
                all_exemplar_tokens.append(cur_level_exemplar_tokens)
        ...
        valid_ratios = valid_ratios.to(torch.bfloat16)
        mask_flatten = mask_flatten.to(torch.bool)
        vision_attention_mask = ~mask_flatten
        # Fourth, sent source_flatten + mask_flatten + lvl_pos_embed_flatten (backbone + proj layer output) through encoder
        # Also provide spatial_shapes, level_start_index and valid_ratios
        if encoder_outputs is None:
            # print("source_flatten: ", source_flatten.shape)
            # print("text_features: ", text_features.shape)
            # print("text_token_mask: ", text_token_mask.shape, text_token_mask)

            used_exemplar_tokens = None
            if len(all_exemplar_tokens) > 0:
                # 5 [456, 234, 123, 567]
                used_exemplar_tokens = all_exemplar_tokens[0]
                # prepare the new mask & pos ids
                exemplar_ids = [30521] * used_exemplar_tokens.shape[1]
                exemplar_ids = torch.tensor(exemplar_ids, dtype=torch.long, device=source_flatten.device)
                exemplar_ids = exemplar_ids.unsqueeze(0)
                all_input_ids = torch.cat([input_ids, exemplar_ids], dim=1)
                # print("all_input_ids: ", all_input_ids.shape, all_input_ids)
                exemplar_text_self_attention_masks, exemplar_position_ids = generate_masks_with_special_tokens_and_transfer_map(all_input_ids)
                # print("exemplar_text_self_attention_masks: ", exemplar_text_self_attention_masks)
                # print("exemplar_position_ids: ", exemplar_position_ids.shape, exemplar_position_ids)
                exemplar_text_attention_mask = torch.ones_like(all_input_ids)
                exemplar_text_attention_mask = exemplar_text_attention_mask.bool()
                exemplar_text_features = torch.cat([text_features, used_exemplar_tokens], dim=1)
                exemplar_text_token_mask = exemplar_text_attention_mask
                exemplar_text_self_attention_masks = exemplar_text_self_attention_masks
            else:
                exemplar_text_features = text_features
                exemplar_text_token_mask = text_token_mask
                exemplar_text_self_attention_masks = text_self_attention_masks
                exemplar_position_ids = position_ids
            # print("exemplar_text_features: ", exemplar_text_features.shape)
            # print("text_features: ", text_features.shape)
            # print("text_token_mask: ", text_token_mask.shape, text_token_mask)
            # print("text_self_attention_masks: ", text_self_attention_masks.shape, text_self_attention_masks)
            # print("position_ids: ", position_ids.shape, position_ids)
            # print("source_flatten: ", source_flatten.shape)
            # print("vision_attention_mask: ", vision_attention_mask.shape, vision_attention_mask)
            # print("lvl_pos_embed_flatten: ", lvl_pos_embed_flatten.shape, lvl_pos_embed_flatten)
            # print("spatial_shapes: ", spatial_shapes.shape, spatial_shapes)
            # print("level_start_index: ", level_start_index.shape, level_start_index)
            ...
            encoder_outputs = self.encoder(
                ...
                text_features=exemplar_text_features,
                text_attention_mask=~exemplar_text_token_mask,
                text_position_embedding=None,
                text_self_attention_masks=~exemplar_text_self_attention_masks,
                text_position_ids=exemplar_position_ids,
                output_attentions=output_attentions,
        ...
        if self.config.two_stage:
            ...
            # print("object_query_embedding: ", object_query_embedding.shape)
            # print("encoder_outputs[1]: ", encoder_outputs[1].shape)
            # print("text_token_mask: ", exemplar_text_token_mask.shape, exemplar_text_token_mask)
            enc_outputs_class = self.encoder_output_class_embed(
                object_query_embedding, encoder_outputs[1], exemplar_text_token_mask
            )
        ...
        decoder_outputs = self.decoder(
            ...
            text_encoder_attention_mask=~exemplar_text_token_mask,
            reference_points=reference_points,
            spatial_shapes=spatial_shapes,
            level_start_index=level_start_index,
            valid_ratios=valid_ratios,
...
class GroundingDinoForObjectDetection(GroundingDinoPreTrainedModel):
    ...
    def __init__(self, config: GroundingDinoConfig):
        ...
        self.negative_fusion_module = None
        self.negative_semantic_extractor = None
    ...
    def forward(
        ...
        **kwargs,
        ...
        # print("out attention_mask: ", attention_mask.shape, attention_mask)
